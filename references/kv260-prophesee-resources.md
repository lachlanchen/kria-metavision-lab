...
## Runtime install & validation status (2026-05-27, local board check)

Commands were run on the KV260 target at IP `<kv260-ip>`.

### Prophesee/Metavision stack present
- `metavision_*` binaries are available in `/usr/bin`:
  - `metavision_viewer`
  - `metavision_software_info`
  - `metavision_file_to_csv`, `metavision_file_info`, `metavision_file_to_video`, etc.
- Device drivers and userspace pipeline are present:
  - modules: `psee_video`, `imx636`, `psee-csi2rxss`, `psee_event_stream_smart_tracker`, `psee_tkeep_handler`, `psee_streamer`

### What I can confirm is visible from the board now
- `load-prophesee-kv260-imx636.sh` executes and reports: `prophesee-kv260-imx636: loaded to slot 0`
- Node set after successful load:
  - `/dev/video0`
  - `/dev/media0`
  - `/dev/v4l-subdev0` … `/dev/v4l-subdev3`
- `media-ctl -p` shows the expected pipeline (`imx636` → `mipi_csi2_rx_subsystem` → `axis_tkeep_handler` → `event_stream_smart_tra` → `ps_host_if output 0`)
- `v4l2-ctl --list-formats-ext -d /dev/video0` shows:
  - `PSE2` (64-bit Prophesee Evt2.1 format)
- Raw stream test succeeds:
  - `v4l2-ctl -d /dev/video0 --stream-count=5 --stream-mmap --stream-to=/tmp/rootcam2.raw`
  - output file size was non-zero (`2,274,488` bytes)

### GUI stack on the current image
- Installed/available:
  - `packagegroup-petalinux-x11` in `rootfs_config`
  - `matchbox-desktop`, `matchbox-terminal`, `matchbox-wm`, `matchbox-session-sato` installed via target `dnf`
- Not available in the current on-device package feed:
  - `xrdp`
  - `xorgxrdp`
- `xserver-nodm` can be started manually when window-manager packages are present.

### What still needs manual download/login (Prophesee customer content)
These are still expected to come from Prophesee customer portals/release files:

- Linux image for microSD flash (`linux image` in kit docs)
- RPM upgrade packets
- SDK with toolchain (`md5: 7397d862bb6c98d7eb64328b8922f51d`)
- Active marker source archive (`rpi-pico-active-marker.zip`, current note says updated 5 months ago at time of capture)

Most of these are gated by Prophesee support/KC account flow shown in:
- `https://support.prophesee.ai/portal/en/kb/articles/starter-kit-amd-kria-kv260`
- `https://support.prophesee.ai/portal/en/kb/prophesee-1/metavision-evks-rdks/embedded-starter-kits/starter-kit-amd-kria-kv260`

### Environment manifest for gated fetch

- `.env` now contains the API endpoints and access mode flags for this KB flow:
  - `PROPHESSEE_KRIA_MAIN_ARTICLE_API` (returns `FORBIDDEN` while unauthenticated)
  - `PROPHESSEE_KRIA_RELEASE_NOTES_API` (currently public)
  - `SUPPORT_AUTH_WORKFLOW`
  - `PROPHESSEE_SUPPORT_HELPER_ARTICLE_DUMP_SCRIPT`
- Use `./scripts/prophesee-support-dump.sh --cookie-file ...` from an authenticated browser session when login artifacts are available.

### Environment manifest

- A consolidated manifest has been added at:
  - `/home/petalinux/Projects/kria-kv260-starter/.env`
- It includes:
  - public links and repository URLs,
  - login-gated artifact checklist,
  - runtime notes (`GUI_PREFERENCE`, `RDP_STATUS`, etc.),
  - placeholder credential variables for local use.

### Practical install sequence for a clean setup
1. On an image/rebuild workflow, enable:
   - `CONFIG_packagegroup-petalinux-x11=y`
   - `CONFIG_packagegroup-petalinux-matchbox=y` (or lighter if needed)
2. Rebuild image and flash.
3. On target:
   - `dnf -y install matchbox-desktop matchbox-terminal matchbox-wm matchbox-session-sato`
4. If/when `xrdp`/`xorgxrdp` become available in your feed, install and enable:
